...
本資料表為金旭共用系統資料同步指示異動記錄檔，用於記錄資料同步指示的異動歷史與狀態變更。
...
|----------|----------|----------|------|------|----------|
| TransState | 交易狀態 | Nvarchar | 1 | M | A:產生 D:刪除 F:失敗刪除 |
| TransDate | 交易日期 | Nvarchar | 8 | M | 格式(YYYYMMDD) |
| Transtime | 交易時間 | Nvarchar | 9 | M | - |
| DS01 | 異動指示識別碼 | uniqueidentifier | - | F | 參照DS資料表(DS01) |
...
- 與[DS(資料同步指示檔)](DS.md)的關聯:

  - DSL.DS01 = DS.DS01
  - 關聯類型: 多對一 (多筆異動記錄對應一筆同步指示)

## 資料表特性說明

以下關鍵組合用於系統內部處理：

- DSL1：DS02+TransDate+Transtime
