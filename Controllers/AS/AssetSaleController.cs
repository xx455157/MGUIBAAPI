...
namespace MGUIBAAPI.Controllers.AS
{
    ...
    /// </summary>
    [Route("as/[controller]")]
    public class AssetSaleController : GUIAppAuthController
    {
        ...
        ///// 商業邏輯物件屬性
        ///// </summary>
        private BlAssetSale BlAssetSale => new BlAssetSale(ClientContent);
        ...
        public MdAssetSale_p GetDataForList(int pageNo, [FromBody] MdAssetSale_q queryParams)
        {
            ...
            return BlAssetSale.GetDataForList(queryParams, pageNo, ControlName, ref _rowsPerPage);
        }

        /// <summary>
        /// 取得明細頁面開檔資料（出售單 Header + 資產清單）
        /// </summary>
        /// <param name="companyId">公司別 (AC01)</param>
        /// <param name="saleNo">出售單號 (AC04)</param>
        ...
        {
            return BlAssetSale.GetDetailData(companyId, saleNo);
        }
        ...
        {
            return BlAssetSale.GetSerialList(companyId, assetNo, purchaseDate);
        }

        /// <summary>
        /// 取得大批出售候選清單（依篩選條件查詢 AA + AB 保管資料）
        /// </summary>
        /// <param name="query">大批出售查詢參數（公司別、資產編號起迄、折舊到期日起迄、保管部門）</param>
        ...
        {
            return BlAssetSale.GetMassSaleList(query);
        }
        ...
        /// 資產出售單新增作業
        /// </summary>
        /// <param name="obj">出售單存檔請求模型</param>
        /// <returns>標準 API 回應訊息（附帶新增的出售單號）</returns>
        [HttpPost("insert")]
        public MdApiMessage Insert([FromBody] MdAssetSale_w obj)
        {
            try
            {
                var _saleNo = BlAssetSale.Insert(obj);
                return HttpContext.Response.InsertSuccess(
                    ...
                    responseData: new
                    {
                        ...
                        saleNo = _saleNo,
                        // 查詢存檔後的結果清單
                        lists = BlAssetSale.GetDataForList(obj.Header.CompanyId, _saleNo)
                    }
                );
            }
            catch (Exception ex)
        ...
        public MdApiMessage Delete(string companyId, string saleNo)
        {
            try
            {
                int _result = BlAssetSale.Delete(companyId, saleNo);
        ...
        public MdApiMessage Update([FromBody] MdAssetSale_w obj)
        {
            try
            {
                var _saleNo = BlAssetSale.Update(obj);
                return HttpContext.Response.UpdateSuccess(
                    ...
                    {
                        // 查詢存檔後的結果清單
                        lists = BlAssetSale.GetDataForList(obj.Header.CompanyId, obj.Header.SaleNo)
                    }
                );
            }
            catch (Exception ex)
