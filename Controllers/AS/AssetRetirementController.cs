...
namespace MGUIBAAPI.Controllers.AS
{
    ...
    /// </summary>
    [Route("as/[controller]")]
    public class AssetRetirementController : GUIAppAuthController
    {
        ...
        ///// 商業邏輯物件屬性
        ///// </summary>
        private BlAssetRetirement BlAssetRetirement => new BlAssetRetirement(ClientContent);
        ...
        public MdAssetSale_p GetDataForList(int pageNo, [FromBody] MdAssetSale_q queryParams)
        {
            var _rowsPerPage = queryParams.RowsPerPage;
            return BlAssetRetirement.GetDataForList(queryParams, pageNo, ControlName, ref _rowsPerPage);
        }

        /// <summary>
        /// 取得明細頁面開檔資料（報廢單 Header + 資產清單）
        /// </summary>
        /// <param name="companyId">公司別 (AC01)</param>
        /// <param name="saleNo">報廢單號 (AC04)</param>
        /// <returns>明細開檔資料，或 null（無資料）</returns>
        [HttpGet("query/detail/{companyId}/{saleNo}")]
        public MdAssetSale_d GetDetailData(string companyId, string saleNo)
        {
            return BlAssetRetirement.GetDetailData(companyId, saleNo);
        }
        ...
        {
            return BlAssetRetirement.GetSerialList(companyId, assetNo, purchaseDate);
        }

        /// <summary>
        /// 取得大批報廢候選清單（依篩選條件查詢 AA + AB 保管資料）
        /// </summary>
        /// <param name="query">大批報廢查詢參數（公司別、資產編號起迄、折舊到期日起迄、保管部門）</param>
        ...
        {
            return BlAssetRetirement.GetMassSaleList(query);
        }
        ...
        /// 資產報廢單新增作業
        /// </summary>
        /// <param name="obj">報廢單存檔請求模型</param>
        /// <returns>標準 API 回應訊息（附帶新增的報廢單號）</returns>
        [HttpPost("insert")]
        public MdApiMessage Insert([FromBody] MdAssetSale_w obj)
        {
            try
            {
                var _saleNo = BlAssetRetirement.Insert(obj);
                return HttpContext.Response.InsertSuccess(
                    ...
                    responseData: new
                    {
                        ...
                        saleNo = _saleNo,
                        // 查詢存檔後的結果清單
                        lists = BlAssetRetirement.GetDataForList(obj.Header.CompanyId, _saleNo)
                    }
                );
            }
            catch (Exception ex)
        ...
        public MdApiMessage Delete(string companyId, string saleNo)
        {
            try
            {
                int _result = BlAssetRetirement.Delete(companyId, saleNo);
        ...
        public MdApiMessage Update([FromBody] MdAssetSale_w obj)
        {
            try
            {
                var _saleNo = BlAssetRetirement.Update(obj);
                return HttpContext.Response.UpdateSuccess(
                    ...
                    {
                        // 查詢存檔後的結果清單
                        lists = BlAssetRetirement.GetDataForList(obj.Header.CompanyId, obj.Header.SaleNo)
                    }
                );
            }
            catch (Exception ex)
